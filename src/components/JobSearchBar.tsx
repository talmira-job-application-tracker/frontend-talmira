...
//           className="w-full sm:w-64 pl-4 pr-4 py-2 rounded-lg shadow-md bg-white/80 placeholder-gray-400 
//                     focus:outline-none focus:ring-2 focus:ring-[#309689] focus:border-[#309689] 
//                     transition duration-300 text-sm sm:text-base"
//         />
...
//       {/* Optional dropdown for suggestions (if you want to show search results as autocomplete) */}
//       {showDropdown && (
...
const JobSearch = () => {
  ...
  const [query, setQuery] = useState("");
  const [location, setLocation] = useState("");
  const [locationInput, setLocationInput] = useState(""); // track input field value
  const [workMode, setWorkMode] = useState("");
  const [showDropdown, setShowDropdown] = useState(false);
  const [typingTimeout, setTypingTimeout] = useState<NodeJS.Timeout | null>(null);
  ...
  useEffect(() => {
    ...
    setTypingTimeout(timeout);
    return () => clearTimeout(timeout);
  }, [query, location, workMode]); // include location & workMode
        ...
        <div className="relative w-full sm:w-64">
          <Autocomplete
            ...
            onPlaceSelected={(place) => {
              ...
              setLocation(loc);       // main filter
              setLocationInput(loc);  // input display
            }}
            types={["(cities)"]}
            placeholder="Location"
            value={locationInput}
            onChange={(e) => {
              ...
              setLocationInput(val);      // update input field
              setLocation(val || "");     // update filter
            }}
            className="w-full pl-4 pr-10 py-2 rounded-lg shadow-md bg-white/80 placeholder-gray-400 
                       focus:outline-none focus:ring-2 focus:ring-[#309689] focus:border-[#309689] 
                       transition duration-300 text-sm sm:text-base"
          ...
          {locationInput && (
            <button
              ...
              onClick={() => {
                setLocationInput("");
                setLocation("");
                handleSearch(); // immediately fetch all jobs
              }}
