          ...
          <thead className="bg-gray-100">
            <tr>
              {["Job", "Candidate", "Email", "Phone", "Date & Time", "Mode"].map((col) => (
                <th
                  key={col}
                  className="px-6 bg-[#309689] py-3 text-left text-white font-semibold uppercase tracking-wide"
                >
                  {col}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-gray-200 text-gray-800">
            {interviews.map((intv) => (
              <tr key={intv._id} className="hover:bg-gray-50 transition-colors cursor-default">
                <td
                  className="px-6 py-4 text-teal-600 font-medium hover:underline cursor-pointer"
                  onClick={() => handleJobClick(intv.application?.job?._id)}
                >
                  {intv.application?.job?.title || "N/A"}
                </td>
                <td className="px-6 py-4">{intv.application?.user?.name || "N/A"}</td>
                <td className="px-6 py-4">
